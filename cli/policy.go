...
func importPolicy(ctx *cli.Context) {
	...
	if node, err := loadPolicy(path); err != nil {
		Fatal("Could not import policy %s: %s\n", path, err)
	} else {
		...
		if resp, err := client.PolicyPut(node.Name, jsonPolicy); err != nil {
			Fatal("Could not import policy directory %s: %s\n", path, err)
...
func dumpPolicy(ctx *cli.Context) {
	...
	if resp, err := client.PolicyGet(path); err != nil {
		Fatal("Could not retrieve policy for: %s: %s\n", path, err)
	} else {
		fmt.Printf("%s\n", resp)
	}
}
...
func deletePolicy(ctx *cli.Context) {
	...
	if err := client.PolicyDelete(path); err != nil {
		Fatal("Could not delete node policy for: %s: %s\n", path, err)
	}
}
...
func verifyPolicy(ctx *cli.Context) {
	...
	if err != nil {
		Fatal("Invalid source: %s\n", err)
	}

	dstSlice, err := parseAllowedSlice(dstInSlice)
	if err != nil {
		Fatal("Invalid destination: %s\n", err)
	}
	...
	if err != nil {
		Fatal("Error while retrieving policy consume result: %s\n", err)
	}
