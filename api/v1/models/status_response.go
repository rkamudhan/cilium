...
type StatusResponse struct {
	...
	// Overall status of Cilium daemon
	...
	// Status of key/value store
	Kvstore *Status `json:"kvstore,omitempty"`

	// Status of logstash integration
	Logstash *Status `json:"logstash,omitempty"`
}
...
func (m *StatusResponse) Validate(formats strfmt.Registry) error {
	...
	if err := m.validateLogstash(formats); err != nil {
		// prop
		res = append(res, err)
	}
...
func (m *StatusResponse) validateLogstash(formats strfmt.Registry) error {

	if swag.IsZero(m.Logstash) { // not required
		return nil
	}

	if m.Logstash != nil {

		if err := m.Logstash.Validate(formats); err != nil {
			if ve, ok := err.(*errors.Validation); ok {
				return ve.ValidateName("logstash")
			}
			return err
		}
	}
