...
func (driver *driver) createEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	_, err := driver.client.EndpointGet("docker-net-endpoint:" + endID)
	if err != nil {
		switch err.(type) {
	...
	log.Infof("New endpoint: container-id=%s", endID)
...
func (driver *driver) joinEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	old, err := driver.client.EndpointGet("docker-net-endpoint:" + j.EndpointID)
	if err != nil {
		sendError(w, fmt.Sprintf("Error retrieving endpoint %s", err), http.StatusBadRequest)
		return
	}
	...
	id := endpointPkg.NewCiliumID(old.ID)
	if err = driver.client.EndpointPatch(id, ep); err != nil {
...
func (driver *driver) leaveEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	if err := driver.client.EndpointDelete("docker-net-endpoint:" + l.EndpointID); err != nil {
