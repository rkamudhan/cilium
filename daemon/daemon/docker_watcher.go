...
func (d *Daemon) createContainer(id string) {
	log.Debugf("Processing create event for docker container %s", id)

	manageThis := false
	maxTries := 5
	containerCopy := types.Container{}

	for try := 1; try <= maxTries; try++ {
		if try > 1 {
			if manageThis = containerCopy.IsDockerOrInfracontainer(); manageThis {
				log.Debugf("Waiting for container %s to appear as endpoint [%d/%d]",
					containerCopy.ID, try, maxTries)
			}
		...
		if ok {
			if !updateOrchLabels(&containerCopy, lbls) {
				// No changes to the orchestration labels
				return
			}
		} else {
		...
		ep = d.lookupDockerID(id)
		if ep == nil {
			// endpoint disappeared in the meanwhile, ignore
	...
	//	if c.NRetries > maxRetries {
	// 		d.DeleteIdentityBySHA256(c.OpLabels.Enabled().SHA256Sum(), c.ID)
