...
func (d *Daemon) lookupCiliumEndpoint(id uint16) *endpoint.Endpoint {
	i := endpoint.NewCiliumID(int64(id))
	if ep, ok := d.endpoints[i]; ok {
		return ep
	} else {
		return nil
	}
...
func (d *Daemon) insertEndpoint(ep *endpoint.Endpoint) {
	...
	id := endpoint.NewCiliumID(int64(ep.ID))
	d.endpoints[id] = ep
...
func (d *Daemon) lookupEndpointLocked(id string) (*endpoint.Endpoint, *apierror.ApiError) {
	prefix, id, err := endpoint.ParseID(id)
	if err != nil {
	...
	case endpoint.CiliumLocalIdPrefix:
		...
		numid, _ := endpoint.ParseCiliumID(id)
		log.Debugf("Fetching cilium id %+v", numid)
		return d.lookupCiliumEndpoint(uint16(numid)), nil
	...
	case endpoint.ContainerIdPrefix:
		return d.lookupDockerID(id), nil

	case endpoint.DockerEndpointPrefix:
		return d.lookupDockerEndpoint(id), nil
...
func (h *getEndpointID) Handle(params GetEndpointIDParams) middleware.Responder {
	...
	log.Debugf("Fetching %+v", params.ID)
...
func (d *Daemon) unlockedRegenerate(epID uint16) error {
	d.endpointsMU.Lock()
	defer d.endpointsMU.Unlock()
	i := endpoint.NewCiliumID(int64(epID))
	ep, ok := d.endpoints[i]
	if !ok {
...
func (h *deleteEndpointID) Handle(params DeleteEndpointIDParams) middleware.Responder {
	d := h.daemon
	log.Debugf("Deleting %+v", params.ID)
...
func (h *putEndpointIDLabels) Handle(params PutEndpointIDLabelsParams) middleware.Responder {
	...
	log.Debugf("Endpoint label change request %+v", params)
