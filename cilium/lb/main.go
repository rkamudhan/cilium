		...
		Subcommands: []cli.Command{
			...
			},
			{
				...
				Flags: []cli.Flag{
					...
					cli.StringSliceFlag{
						Name:  "backend",
						Usage: "Backend address, port and optional weight: <IPaddress>:<port>[,weight]",
					},
					cli.IntFlag{
						Name:  "id",
...
func cliDumpServices(ctx *cli.Context) {
	...
	for _, v := range dump {
		...
		for i, be := range v.BES {
			var str string
			if be.Weight != 0 {
				str = fmt.Sprintf("%d => %s (W: %d, ID: %d)", i+1, be.Addr.String(), be.Weight, v.FE.ID)
			} else {
...
func cliDumpWrr(ctx *cli.Context) {
	...
	for _, v := range dump {
		str := fmt.Sprintf("(count %d) seq%v", v.SEQ.Count, v.SEQ.Idx[:v.SEQ.Count])
